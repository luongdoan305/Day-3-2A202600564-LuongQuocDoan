...
def get_tools_v1() -> List[Dict[str, Any]]:
    ...
    return [
        _spec(
            "search_hotels",
            "Search real hotels and room prices from an external hotel API. "
            "Input JSON: {city: string, nights: integer}. "
            "Output: A list of hotels with name, price_per_night, rating, address, nights, and total_price. "
            "Use this tool when the user asks about hotel, accommodation, room price, hotel cost, or travel budget.",
            search_hotels,
            'search_hotels("Đà Nẵng", 2)',
        ),
        _spec(
            "get_weather",
